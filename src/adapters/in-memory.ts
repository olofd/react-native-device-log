...
export default class InMemory {
   ...
   */
  async setItem(key: string, value: any, callback: () => any): Promise<any> {
   ...
   */
  async getItem(key: string, callback: Function): Promise<any> {
    return await this.promisify(() => {
   ...
   */
  async removeItem(key: string, callback: Function): Promise<any> {
    return await this.promisify(() => {
  ...
  promisify(cb: { (): void; (): any; (): void; (): unknown }) {
